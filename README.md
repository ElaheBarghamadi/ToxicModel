...
p ≥ 0.478  →  🚫 block (نامناسب)
در غیر این صورت → ✅ ok (مجاز)
```

### ⚔️ مقایسه الگوریتم‌ها (پروتکل یکسان: همان ۱۹٬۸۰۰ داده، همان ۶ تست، آستانه هر مدل از validation)

| مدل | توییتر | تلگرام | ParsOff | PHate | PHICAD | کامنت سایت | **کل (mixed)** |
|---|---|---|---|---|---|---|---|
| **LogisticRegression (استقرار)** | 0.734 | 0.813 | 0.732 | 0.695 | 0.890 | 0.658 | **0.855** |
| LinearSVC | 0.740 | 0.815 | 0.730 | 0.700 | 0.888 | 0.658 | 0.855 |
| MultinomialNB | 0.644 | 0.805 | 0.706 | 0.683 | 0.894 | 0.683 | 0.850 |
| آنسامبل رأی‌گیری نرم (LR+SVC+NB) | 0.726 | 0.817 | 0.729 | 0.706 | 0.898 | 0.650 | **0.862** |

> **حکم:** تفاوت بهترین جایگزین (آنسامبل، 0.862) با لجستیک (0.855) در مرز فاصله اطمینان است — برتری معنادار ندارد. مدل استقرار **LogisticRegression** می‌ماند (الزام تکلیف + هم‌ترازی عملکرد + سبکی و احتمال کالیبره). مدل آنسامبل در `model_alt.joblib` برای مقایسه نگه داشته شده است. سقف واقعی بهبود، مدل‌های زمینه‌ای (ParsBERT و مشابه) است که خارج الزام و امکانات این محیط است.
